"""
FILE:            shot_noise.py
SW-COMPONENT:    Corruption script of shot_noise
DESCRIPTION:     Script containing a class for corrupting an image with shot noise
...
class ShotNoise(BaseCorruption):
    """
    This code corrupts an image with shot noise
    ...
    def __call__(self, image, severity=1):
        ...
        # Apply shot noise corruption and clip it to [0-1] range
        corrupted_image = np.clip(np.random.poisson(x * c) / float(c), 0, 1).transpose((2, 0, 1))
