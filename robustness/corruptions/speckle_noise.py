"""
FILE:            speckle_noise.py
SW-COMPONENT:    Corruption script of speckle_noise
DESCRIPTION:     Script containing a class for corrupting an image with speckle noise
...
class SpeckleNoise(BaseCorruption):
    """
    This code corrupts an image with speckle noise
    ...
    def __init__(
        ...
        """
        Creates an SpeckleNoise instance
        """
        super().__init__()
    ...
    def __call__(self, image, severity=1):
        ...
        # Apply speckle noise corruption and clip it to [0-1] range
        corrupted_image = np.clip(x + x * np.random.normal(size=x.shape, scale=c), 0, 1).transpose((2, 0, 1))
