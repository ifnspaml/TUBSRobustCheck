"""
FILE:            metzen.py
SW-COMPONENT:    Adversarial attack script of METZEN attack
DESCRIPTION:     Script containing a class for creating a metzen attack
...
class Metzen(BaseAttack):
    """
    This code implements the Metzen of the paper
    ...
    def __init__(
        ...
        # Check if the tile sizes are valid
        ...
        # Create variable to store adv perurbation
    ...
    def __call__(self, x, model, t=None, y=None):
        """
        Implementation of METZEN method.
        ...
        self.xi_resized = torch.zeros(images.shape).cuda()
        ...
        for epoch in range(self.iterations):
            ...
            self.xi = self.xi - (self.alpha * torch.sign(grad_x.cuda()))
        ...
        # Check whether the adversarial perturbation has already been computed
        # and if images and models are on the desired device
        self.check_call(x, model)

        # Clamp adversarial example
        adv_ex = torch.clamp(x + self.xi_resized, 0, 1)

        return adv_ex, self.target_label
    ...
    def _load_target_label(self):
        ...
        # Reshape it to a single dimensiona array
        lbl = lbl.reshape(-1)
    ...
    def _loss_gradient(self, model, pert, target):
        """
        This function computes the loss for the Metzen attack.

        :param model: The model which is attacked
        :param pert: the perturbed image
        ...
        # Requires that the gradient are computed
        pert.requires_grad = True

        # Forward the perturbed image to the model
        output = model(pert)
        ...
        # View tensor in a different way: [Batch, C, Patch, patch height, patch width]
        ...
        for i in range(patches.size()[1]):
            ...
            # Compute the losses for the dynamic case
                ...
                # Subtract the mininum value, divide by the max (normalization)
    ...
    def _output_without_class(self, target, target_class):
        ...
        :param target_class: The target class which should be replaces in the image target.
